...
func TestPatch_Success(t *testing.T) {
	...
	setTrue := true
	task, err := service.Patch(1, &newTitle, &setTrue, nil, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestPatch_NothingToUpdate(t *testing.T) {
	...
	_, err = service.Patch(1, nil, nil, nil, nil)
...
func TestPatch_InvalidID(t *testing.T) {
	...
	_, err = service.Patch(0, nil, nil, nil, nil)
	if !errors.Is(err, model.ErrInvalidID) {
		t.Fatalf("expected ErrInvalidID, got %v", err)
	}
}
...
func TestPatch_Title(t *testing.T) {
	...
	task, err := service.Patch(1, &newTitle, nil, nil, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestPatch_Done(t *testing.T) {
	...
	newTrue := true
	task, err := service.Patch(1, nil, &newTrue, nil, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if task.Done != true {
