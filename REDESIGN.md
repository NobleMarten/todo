...
- `hooks/useTasks.ts`: `useTasks(spec)` (оптимистично с откатом, выполненная/ушедшая из вида задача пропадает после
  ...
  `useWeekProgress`. `hooks/useProjects.ts` — активные + архивные, CRUD (перетаскивания списков нет — см. макеты ниже).
...
Сверка с макетами `design/` (B1_Spiski, B2_Proekt, C2_Karta), вторым коммитом этапа:
- Взяты структура, отступы, размеры шрифтов и формулировки: «Списки» + «N активных», смарт-карточки с иконкой и
  счётчиком, «мои списки», бейдж просроченных — пилюля; шапка списка на всю ширину с прогрессом и чипами; подписи
  секций «просрочено · 1»; справа в строке бейджи `DD.MM` / `1/3`, у задачи без дат — календарь «назначить дедлайн»
  (ставит `due_date`); раскрытие — по бейджу `1/3`, подзадачи с квадратными чекбоксами, «до DD.MM» и «+ подзадача»;
  снизу пристыкованное поле «задача в Go…» вместо таб-бара; карточка — «задача · #id», чекбокс + заголовок, чип списка
  в его цвете, сегмент приоритета моноширинным, два поля даты строками («4 октября, сб», «через 13 дн»), «1 / 4» с
  полосой цвета списка, «добавить подзадачу», кнопки 48 px «выполнено» + квадратная корзина.
- Полоса прогресса и точка — цветом списка (`--tint` из данных), остальное — токены `theme.css`, светлая тема сохранена.
- Таб-бар скрыт на `/lists/:id`, кроме `/lists/week` (он же пункт «неделя»): в B2 его нет, снизу поле ввода.

Расхождения макетов со спекой (сделано по REDESIGN.md):
- B1: кнопка поиска — поиск вне объёма (раздел 7), не рисуется; на её месте переключатель темы. Логотипа `[todo]` в
  макетах нет — оставлен над заголовком (раздел 5). Таб «итоги» — Этап 4. Строки «входящие» и «в архиве» в макете нет.
- B2: чип «доска» не рисуется (раздел 5). Цвет чекбокса «обычно» в макете — `--accent`, по спеке — `--faint`.
  Строка задачи в макете ~44 px, по спеке ≥ 56; кнопки 32×32 из макета — видимая плашка внутри зоны 44×44; мелкие
  чипы и бейджи добирают касание псевдоэлементом. Ручки перетаскивания в макете нет — без неё на телефоне не отличить
  drag от прокрутки, оставлена приглушённой.
- C2: чекбокс в карточке в макете цвета списка, по спеке — цвета приоритета. Кнопка «ещё действия» не рисуется (нечего
  в неё положить; удаление — внизу). «Выполнено» с заглавной в макете — по спеке ярлыки в нижнем регистре.
  Даты: у «делаю» в макете формат «среда, 23 сентября», у дедлайна «4 октября, сб» — для обоих взят второй; у заданной
  даты справа крестик «убрать» вместо шеврона (в макете способа очистить дату нет).
- Не по спеке, но и не из макета: заголовки экранов и смарт-карточек с заглавной («Списки», «Сегодня») — так в
  макете и в тексте раздела 5; ярлыки, чипы, кнопки — строчные.

### Этап 4 — вариант А · 2026-09-24 · ветка `redesign/stage-1`, коммит «этап 4: …»
Сделано:
- `/today` — экран по умолчанию (`TodayScreen`, макет A1): дата «чт · 24 сентября», «Сегодня» + `выполнено / (выполнено +
  в плане)` с полосой, «фокус дня» = `planned[0]`, красный блок «просрочено · N» = `overdue` + `carry_over` с одной кнопкой
  «перенести на сегодня» (один `POST /day/plan`), «план на день · N» (остальные planned, перетаскивание через
  `/tasks/reorder` со scope `day`; фокус остаётся первым), строка «+ добавить из списков» → `/plan`, свёрнутое
  «готово · N» из `done_today` (снять галочку — вернуть). Пустой план — пустое состояние с кнопкой «собрать день».
- `/plan` — `PlanDayScreen` (макет A2): «просрочено», «дедлайн на этой неделе», «из списков · давно не трогал»
  (N дней от `updated_at`). Выбор локальный (`+` / галочка), «начать день · N задач», N = уже в плане + выбрано;
  один `POST /day/plan` (`remove` всегда пуст — сервер не предлагает уже запланированное) и переход на `/today`.
  Таб-бар на `/plan` скрыт, снизу пристыкована кнопка.
- `/archive` — `ArchiveScreen` («Итоги»): грид `Activity.tsx` без изменений + выполненные по дням `done_at`
  («сегодня», «вчера», «пн · 21 сентября») со временем, `view=archive&sort=done_at&order=desc`, по 50, «показать ещё · N».
  Таб «итоги» — четвёртый пункт таб-бара (иконка-график из макета).
- `useActivity` переписан на `GET /stats/activity` (`from=2000-01-01` — статистика «всего/макс. серия» за всё время,
  ответ разреженный), перечитывается по шине `lib/sync`. `lib/format.localDayOf` удалён (больше не нужен).
- Новое: `hooks/useDay.ts` (`useDay` — оптимистично с откатом, после ответа день перечитывается целиком; при возврате
  на вкладку в новый день грузит новый день; `useSuggestions`), `useArchive` в `useTasks.ts`, `api/day.ts`
  (`getDay`, `getSuggestions`, `getActivity`), типы `Day`/`Suggestions`/`DayCount`, `components/DoneRow.tsx`,
  `components/TaskRun.tsx` (перетаскиваемый отрезок вынесен из `ProjectScreen` без изменений), `TaskRow.alert`
  (красная рамка у «вчера не доделал» без дедлайна), `lib/date.dayHeading/timeOf`, `ChartIcon`.
- Прямая ссылка `/task/:id` теперь открывает карточку поверх «Сегодня», закрытие ведёт на `/today`.
- Бэкенд не менялся.
- Проверено: Go-приёмка, `npm run build`, `npm run lint`, разовый `tsc --strict --noEmit` через npx; ручной прогон
  390×844 в headless Chrome (playwright-core) против локального API на одноразовой базе: выполнение фокуса и возврат,
  drag плана (порядок в БД), «перенести на сегодня», карточка поверх «Сегодня» и синхронизация после «выполнено»,
  «Собрать день» → `/today` с 7 задачами, «Итоги» (показать ещё, вернуть), пустое состояние, светлая тема,
  горизонтального переполнения нет.

Отклонения от спеки и решения за владельца:
- «перенести на сегодня» — для всего красного блока (`overdue` + `carry_over`), а не только `carry_over`: в макете там
  задача с просроченным дедлайном, два красных блока подряд не нужны.
- Фокус не повторяется в «плане на день»; сменить фокус перетаскиванием нельзя — он меняется, когда фокус выполнен.
- `/plan` — полноэкранный экран с «назад», а не шит с «ручкой» из макета (закрыть его жестом всё равно нечем).
- Из A1 не рисуются «~90 мин» (оценки времени в модели нет) и поиск (вне объёма). Чекбокс фокуса — цвета приоритета
  (в макете — акцент). Бейджи строк — `DD.MM`, как на Этапе 3, а не «завтра». «Начать день» — строчными.
- Грид активности теперь делит дни в `APP_TZ` (сервер), а не по часам устройства; время в «Итогах» — локальное.
- `POST /tasks/clear` на фронте **не сделан** (решение владельца): бэкенд делает `TRUNCATE tasks` — стирает и активные,
  кнопка на экране «итоги» вводила бы в заблуждение.

Долги, которые тянутся дальше:
- `/tasks/clear` без UI — решить на Этапе 5: кнопка с «точно? / точно-точно?» или убрать эндпоинт.
- `Activity.tsx` не трогал (раздел 7): в ошибке там англоязычное «↻ retry», статистика на 390 px переносит 4-й блок
  на вторую строку, `empty-icon/empty-title/empty-hint` без стилей — полировка Этапа 5.
- Выполненная из карточки задача остаётся в открытой карточке; «Сегодня» под ней обновляется по шине — ок, но
  шина по-прежнему дёргает все подписанные хуки (на «Итогах» — активность + до 200 задач архива).
- Смена суток при открытом экране ловится только по `visibilitychange`; «Собрать день» берёт дату при открытии.
- В headless-прогоне одна ошибка консоли `ERR_SOCKET_NOT_CONNECTED` — загрузка внешнего ресурса (Google Fonts),
  к приложению не относится.
- Деплой — как после Этапа 3: `frontend/nginx.conf` с `/tasks|/projects|/day|/stats` и `COPY migrations` (`OWNER_TODO.md`).
- `CLAUDE.md` и README описывают старое приложение — Этап 5.

### Этап 5 — полировка · 2026-09-24 · ветка `redesign/stage-1`, коммит «этап 5: …»
Сделано:
- Свайпы (`components/SwipeRow.tsx`, framer-motion `drag="x"`): влево — открывается красная «удалить», нажатие на неё =
  подтверждение; вправо за 80 px — «на сегодня» (в списках `PATCH scheduled_for`, в красном блоке «Сегодня» — `POST /day/plan`).
  Подключены в `TaskRow` через `onDelete/onToday`; у задачи, уже стоящей на сегодня, и у выполненной правого свайпа нет.
  Одна открытая строка за раз, касание мимо закрывает, клик после жеста гасится (и по флагу drag, и по сдвигу > 8 px).
  `useDay.remove` — новое. Фокус дня, «Собрать день», «готово» и подзадачи без свайпов.
- Быстрый ввод: `lib/quickAdd.ts` (`#список` без регистра и с опущенными пробелами, `!срочно|!важно|!обычно`,
  `ДД.ММ` / `ДД.ММ.ГГ(ГГ)` / `завтра` / `пн…вс` → `due_date`; первое совпадение каждого вида, остальное — заголовок).
  `QuickAdd` показывает распознанное чипами и на 2,5 с — «добавлено в «X»», если задача ушла из текущего вида.
- Скелетоны (`Skeleton.tsx`) вместо спиннеров первой загрузки на всех экранах и в гриде активности; `ErrorState.tsx` —
  «не удалось загрузить · повторить», когда показать нечего; `ErrorBoundary.tsx` вокруг приложения («перезагрузить»);
  пустые состояния с подсказкой (в пустом списке — синтаксис быстрого ввода). `errorText` переводит коды API
  (бэкенд шлёт английские message), сеть и 5xx на русский.
- iPhone/PWA: `index.html` — `lang="ru"`, `viewport-fit=cover` (без него все `env(safe-area-inset-*)` были 0),
  `apple-mobile-web-app-capable`, `black-translucent`, `theme-color` (синхронизируется с темой в `App.tsx`);
  `manifest.json` — `start_url: /today`, `scope`, цвета `#0B0B0F`; шит не заезжает под статус-бар; боковые safe-area;
  `hooks/useAutosize.ts` — высота заголовка и заметки карточки там, где нет `field-sizing` (Safari < 26).
- `Activity.tsx` минимально: «↻ retry» → «повторить», скелетон; CSS — стили `empty-*`, статистика сеткой в 4 колонки.
- `README.md` переписан под новое API, `CLAUDE.md` — под новую архитектуру.
- Проверено: Go-приёмка (`-count=1`), `npm run build`, `npm run lint`, разовый `tsc --strict`; сквозной прогон в Playwright
  WebKit с профилем iPhone 13 (390×844, touch) против локального API на одноразовой базе — 30 проверок зелёные: свайпы
  (вправо/влево, до сервера, без открытия карточки), ручка перетаскивания не сдвигает строку вбок и меняет порядок в БД,
  разбор и чипы быстрого ввода, скелетон, ErrorState и «повторить», русский TASK_NOT_FOUND, горизонтального переполнения нет,
  theme-color в обеих темах.

Отклонения от спеки и решения за владельца:
- `ДД.ММ` без года в прошлом → следующий год; `пн` в понедельник → следующий понедельник; `сегодня` не ключевое слово.
- Подтверждение удаления — кнопка под строкой, а не диалог и не «отменить».
- Свайп стартует вручную через `dragControls` + `dragPropagation`: framer не начинает drag на вложенной `<button>`,
  а глобальная блокировка framer срывала следующий свайп. Ручка перетаскивания слушает `pointerdown` нативно
  со `stopPropagation`, иначе вместе с ней ехала бы строка вбок.
- `POST /tasks/clear` оставлен без UI (стирает и активные задачи).
- Реальный iPhone не проверялся — только эмуляция WebKit; чек-лист — в отчёте этапа.

Долги, которые тянутся дальше:
- Проверить на устройстве в режиме PWA: отступы под чёлкой/home-индикатором, свайп против вертикальной прокрутки
  (`touch-action: pan-y`), нативный date-picker в строке, зум при фокусе полей (все ≥ 16px), статус-бар в светлой теме.
  Иконка PWA по-прежнему `pic.png`/`pic3.png` — если уже установлена, iOS обновит `start_url` только после переустановки.
- Правый свайп и `POST /day/plan` на задаче с просроченным дедлайном не убирают её из «просрочено» — это по данным
  (дедлайн прошёл), так и задумано, но может удивить.
- Нет фронтовых тестов (`lib/quickAdd.ts` проверен разово в node); стоит завести vitest вместе с tsconfig.
- Из прежних: `//go:build ignore` в `_legacy/cmd/todo-cli`; 404/405 самого ServeMux — plain text; тело > 1 МБ → 400, а не 413;
  `model.ErrNotAllowed` не используется; шина `lib/sync` перечитывает все подписанные хуки после любой мутации.

#### Этап 5, доработка по отзыву владельца · 2026-09-24
- «#список не работает»: сопоставление было слишком строгим (точное имя). Теперь `findProject` в `lib/quickAdd.ts`:
  без регистра, ё = е, без пробелов/дефисов/знаков (`#учеба,` → Учёба), однозначное начало имени (`#fin` → Finance-Tracker).
  При наборе `#…` под полем — подсказки списков, нажатие дописывает тег (фокус и клавиатура остаются).
  Точную причину на устройстве воспроизвести не удалось — если не заработает, нужен пример ввода и имя списка.
- Выполненные в «папке»: `components/DoneSection.tsx` — свёрнутое «выполнено · N» внизу списка и входящих
  (`view=archive&project_id=` / `view=inbox&done=true`, `sort=done_at desc`, по 50 + «показать ещё»), счётчик коротко
...
### Этап 8 — «вернуть» после удаления, день работы в быстром вводе · 2026-09-25 · ветка `redesign/stage-1`, коммит «этап 8: …»
Сделано:
- `lib/pendingDelete.ts`: одно отложенное удаление — задача прячется сразу, `DELETE` через 5 с; следующее удаление
  или уход приложения в фон (`visibilitychange: hidden`, `pagehide`) проводят его сразу; «вернуть» отменяет;
  ошибка сервера — задача возвращается и внизу сообщение (4 с); 404 `TASK_NOT_FOUND` = успех (удалена каскадом).
- `lib/deleting.ts` — множество спрятанных id; фильтр стоит в `api/tasks.listTasks/getTask` и `api/day.getDay/getSuggestions`
  (total уменьшается на спрятанные), поэтому перечитывания по шине не воскрешают задачу. «Вернуть» = убрать из множества +
  `notifyChanged()` всем — откатов в хуках не нужно.
- `useTasks.remove`, `useDay.remove`, `useTask.remove/removeSubtask` переведены на него; `components/UndoToast.tsx` —
  «удалено «…» · вернуть», один на приложение, над таб-баром / полем ввода.
- Быстрый ввод: `@сегодня`, `@завтра`, `@пн…@вс`, `@ДД.ММ(.ГГ)` → `scheduled_for`; дата без `@` — по-прежнему дедлайн.
  Чип «делаю DD.MM / сегодня», подсказка в пустом списке. В «Сегодня» `@день` важнее автоматического «сегодня».
- Тесты: `lib/pendingDelete.test.ts` (фейковые таймеры: задержка, отмена, «следующее проводит предыдущее», ошибка, 404,
  подзадачи), `@` в `quickAdd.test.ts`. Всего 102.
- Проверено: `tsc`, lint, `npm test`, build; Go не менялся.

Отклонения и решения за владельца:
- Кнопка «удалить» после свайпа влево осталась (свайп открывает, нажатие удаляет): полный свайп-удаление слишком легко
  сделать случайно при прокрутке. Изменилось то, что после нажатия есть 5 с на «вернуть».
- Удалённая с отменой задача, пока висит тост, не видна нигде (фильтр в api), но счётчики со `limit=1`
  (смарт-карточки «Списков») могут на эти секунды показывать на 1 больше — уточнятся после удаления.
- Закрыли вкладку за эти 5 с — задача остаётся (так договорились).

Долги:
- `@` работает только в одном поле быстрого ввода (списки / входящие / сегодня); на экране «Неделя» — см. Этап 9.

### Этап 9 — «Неделя» · 2026-09-25 · ветка `redesign/stage-1`, коммит «этап 9: …»
Сделано:
- Бэкенд: `GET /day/week?from=` (`service/week_service.go`, `DayService.Week`) — из существующих условий `TaskFilter`
  (`AnyDateBetween`, `DoneBetween`, `DueBetween`, `NoDates`), репозитории не менялись. Без `from` — понедельник текущей
  недели в `APP_TZ`; с `from` — семь дней с него как есть. День: `scheduled`, `deadlines` (due = день и не запланирована
  на этот же день), `done` (по `done_at` в APP_TZ). `upcoming` — до 6 дедлайнов в 60 днях после недели; `backlog` — до 30
  без дат + `backlog_total`. Пустые списки — `[]`. Тесты: `TestWeek`, `TestMondayOf`, `TestWeekHandler`.
- Фронт: `screens/WeekScreen.tsx` (макет C1) на `/week`, `?from=&day=` в URL (назад/вперёд и карточка поверх работают);
  таб «неделя» теперь ведёт сюда (раньше на `/lists/week`, смарт-вид «7 дней» остался в «Списках»). Полоса пн–вс прилипает
  к верху, до трёх точек цвета списков, выбранный день — акцент, сегодня — рамка, прошлые приглушены; стрелки, свайп
  по полосе и чип «сегодня» листают недели. Список дня: дедлайны дня (красная рамка, если день ≤ сегодня) + запланированные
  (`TaskRow` со свайпами) + выполненные (`DoneRow`). Встроенное поле `QuickAdd inline` — задача на выбранный день
  (`@дата` важнее). «Дедлайны впереди» — карточки в ряд («11.10 · 16 дней», ≤ 14 дней — красным). Бэклог — чипы.
- Перетаскивание на день: `components/DayDrag.tsx` (framer `drag` + `dragSnapToOrigin`, день под пальцем через
  `elementsFromPoint` → `[data-day]`, подсветка ячейки). Строки — за ручку (как сортировка; свайп строки не мешает),
  чипы бэклога — целиком (`touch-action: none`), нажатие на чип — на выбранный день. Результат — `PATCH scheduled_for`.
- `hooks/useWeek.ts` (оптимистично с откатом, перечитывание по шине, прошлая неделя видна, пока грузится новая),
  `api.getWeek` (с фильтром удаляемых), типы `Week/WeekDay`, `lib/date`: `mondayOf`, `longDayLabel`, `weekTitle`,
  `dayOfMonth`, `weekdayShort` + тесты.
- Проверено: Go-приёмка, lint/tsc/test (104)/build; сквозной прогон WebKit iPhone 13 против локального API на одноразовой
  базе через same-origin прокси с `APP_PASSWORD` — 36 проверок: вход (неверный/верный пароль, выход → 401), полоса, точки,
  дедлайны впереди, тап по чипу бэклога, перетаскивание чипа и строки за ручку на день (проверено в БД), `@ДД.ММ` и день
  по умолчанию во встроенном поле, удаление с «вернуть» и удаление через 5 с (в БД), листание недель и «сегодня»,
  без горизонтального переполнения, без ошибок в консоли; скриншоты тёмной и светлой темы.

Отклонения и решения за владельца:
- Бэклог — секция в конце экрана, а не пристыкованная шторка из макета: снизу уже таб-бар, а полоса дней прилипает к верху,
  поэтому тащить из бэклога вверх можно при любой прокрутке.
- Переупорядочивания внутри дня на «Неделе» нет (ручка тащит на другой день); порядок дня — на «Сегодня».
- Бросок строки дедлайна на день ставит «делаю» (`scheduled_for`), дедлайн не двигается.
- `@ДД.ММ` без года в прошлом уходит на следующий год, как и дедлайны (правило `lib/quickAdd`); для прошлых дней недели
  есть перетаскивание и поле дня.
- Светлая тема: цифры выбранного дня белые (акцент светлой темы темнее).

Долги:
- На iPhone проверить: перетаскивание чипа бэклога не мешает прокрутке (чипы с `touch-action: none`), прилипание полосы
  под чёлкой в режиме PWA.
- Задача с бейджем «делаю DD.MM» в списке дня повторяет дату дня — `TaskRow` не знает, что он на «Неделе».

### Этап 10 — повторяющиеся задачи · 2026-09-25 · ветка `redesign/stage-1`, коммит «этап 10: …»
Сделано:
- Миграция `00007_tasks_repeat` (`tasks.repeat TEXT`, NULL — не повторяется; Down удаляет колонку — теряются только правила).
- `model.Repeat` (`model/repeat.go`): `daily | weekdays | weekly:1,4 | monthly:15`, `ParseRepeat` (канон: дни недели
  по возрастанию без повторов), `Matches`, `After`, `DaysBetween`; `ErrInvalidRepeat` → 400 `INVALID_REPEAT`.
  `monthly:31` в коротком месяце — последнее число.
- Репозитории: `repeat` во всех выдачах задач, в `NewTask` (+ `Note`) и `TaskPatch`; контрактный тест на обоих.
- Сервис: правило проверяется и приводится к канону при создании и PATCH; у подзадач повтора нет (в т. ч. нельзя
  сделать повторяющуюся подзадачей). `PATCH done:true` у повторяющейся: у выполненной правило снимается, `spawnNext`
  создаёт следующую (заголовок, приоритет, список, заметка, правило; без подзадач). `nextDates`: опора — «делаю»,
  иначе дедлайн, иначе сегодня; следующий день по правилу, но не раньше сегодня; обе даты сдвигаются на одно число дней;
  без дат — «делаю» = день по правилу. Не создалась копия — выполнение откатывается.
- Фронт: `lib/repeat.ts` (разбор, подписи «каждый день / по будням / по пн, чт / каждое 15-е», `onOrAfter`, варианты поля),
  быстрый ввод — `ежедневно | по-будням | еженедельно | ежемесячно` (неделя/месяц — от даты задачи или сегодня; задача без
  дат получает «делаю» = ближайший день по правилу), чип «↻ …»; `components/RepeatField.tsx` в карточке (как поля дат,
  нативный `select`, для «по дням недели» — переключатели пн…вс, крестик «убрать повтор»); значок ↻ у строки;
  `useTasks.update` после выполнения повторяющейся перечитывает свой вид (копия появляется сразу).
- Попутно (баг с Этапа 3): пустое значение поля даты в карточке («не задано») получало класс `.empty` — тот же, что у пустых
  состояний экранов (пунктирная рамка, отступы 32 px), и поле «дедлайн» раздувалось. Модификатор переименован в `.is-empty`.
- Тесты: Go — `model/repeat_test.go`, `TestNextDates`, `TestRepeatOnDone`, `TestRepeatValidation`, `TestRepeatHandler`,
  контракт; фронт — `lib/repeat.test.ts`, повтор в `quickAdd.test.ts` (всего 124). Сквозной WebKit: 14 проверок повтора
  (быстрый ввод → правило и «делаю», выполнение → копия на завтра видна без перезагрузки, выполненная без правила,
  карточка: смена на «каждую неделю», второй день, «убрать повтор», INVALID_REPEAT) + прежние 36 проверок «Недели».
- Сценарий e2e: жесты в тестах теперь не быстрее кадра — framer читает движение по rAF, и свайп из 10 движений без пауз
  в headless WebKit иногда не начинался (у живого пальца так не бывает; к коду не относится).

Отклонения и решения за владельца:
- Модель «следующая копия при выполнении», а не расписание заранее: в списках одна строка на правило, история — в «Итогах».
- Копия встаёт в конец списка (`MAX(position)+1`), а не на место выполненной.
- `еженедельно`/`ежемесячно` в быстром вводе — от даты задачи; нужны несколько дней недели — в карточке.
- Выполненная с опозданием не плодит просроченные копии: следующая — не раньше сегодня.

Долги:
- Создание копии и выполнение — два запроса к БД без общей транзакции (компенсация при ошибке есть, но окно гонки остаётся).
- Откат на проде: `goose down-to 0` теперь проходит 7 миграций; Down `00007` удаляет правила повтора.

### Этап 11 — поиск · 2026-09-25 · ветка `redesign/stage-1`, коммит «этап 11: …»
Сделано:
- Бэкенд: `GET /tasks?q=` в любой вьюхе — `TaskFilter.Search` → `(t.title ILIKE @q OR t.note ILIKE @q)`, `%`, `_`, `\`
  экранируются; `FakeRepo` — `strings.Contains` по `ToLower`. Только корневые задачи (как все выдачи). `q` обрезается по
  краям, пустой — не фильтрует, длиннее 100 символов — 400 `INVALID_QUERY`. Новых префиксов нет — nginx не трогать.
  Тесты: контракт (кириллица в разном регистре, заметка, `%` и `_` буквально, подзадачи не находятся, выполненные
  отдельно) на обоих репозиториях, `TestSearchHandler`.
- Фронт: `screens/SearchScreen.tsx` на `/search` (кнопка-лупа в шапке «Списков» рядом с темой; таб «списки» подсвечен),
  `hooks/useSearch.ts` — дебаунс 250 мс, два запроса параллельно: активные (`view=all&q`) и выполненные
  (`view=archive&q`, свежие, до 30 + общее число), устаревшие ответы отбрасываются, перечитывание по шине. Запрос в `?q=` —
  карточка поверх и «назад» возвращают к тем же результатам. Из результатов можно выполнить/вернуть и назначить дедлайн.
- Проверено: Go-приёмка + контракт на Postgres, lint/tsc/test (124)/build; сквозной WebKit 13 проверок поиска и повторный
  прогон всех сценариев этапов 7–11 (36 + 14 + 13) — зелёные.

Отклонения и решения за владельца:
- `ILIKE` без индекса и без полнотекстового поиска: на сотнях задач — миллисекунды.
- Регистр кириллицы зависит от `lc_ctype` базы: локально (en_US.UTF-8) работает; на VPS проверить одной командой
  (`OWNER_TODO.md`, п. 10а), при `C` — перейти на `lower()` с ICU.
- `ё` и `е` в поиске различаются (в `#списке` быстрого ввода — нет).
- Подзадачи в поиск не попадают: результатом всегда служит корневая задача, её подзадачи — в карточке.

Долги этапов 7–11 целиком:
- **Деплой**: `OWNER_TODO.md` п. 6–11 — nginx `auth`, `APP_PASSWORD`, cron бэкапа, мерж, проверка локали, iPhone.
- На реальном iPhone не проверялось ничего из этапов 7–11 (только WebKit с профилем iPhone 13).
- Без HTTPS пароль идёт открытым текстом.
- Создание копии повторяющейся задачи и выполнение — не одна транзакция.
