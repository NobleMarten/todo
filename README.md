...
- `/day/week` — экран «Неделя»: семь дней с `from` (без него — с понедельника текущей недели в `APP_TZ`).
  В дне: `scheduled` — `scheduled_for = date`, `deadlines` — `due_date = date` (если не запланирована на этот же день),
  `done` — выполненные в этот день. Задача с датами в разных днях показывается в обоих. `upcoming` — до 6 дедлайнов
...
- **Удаление с «вернуть»**: задача пропадает сразу, внизу 5 с висит «удалено · вернуть», запрос уходит после
  (или сразу, если приложение свернули / удалили следующую). Так же — удаление из карточки и подзадач.
- **Неделя** (`/week`, таб «неделя»): полоса пн–вс с точками цвета списков (свайп вбок и стрелки листают недели),
  список выбранного дня, «дедлайны впереди» и бэклог. Задачу тащат на день полосы — строку за ручку, чип бэклога
